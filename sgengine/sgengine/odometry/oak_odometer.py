...
# From OpenVO: https://github.com/justincdavis/openVO


@jit
def bilinear_interpolate_pixels(img, x, y):
...
# Paper alg
def rigid_body_filter(prev_pts, pts, rigidity_threshold):
...
class OAK_Odometer:
    ...
    def __init__(
        ...
        # TODO crosscheck
        self.orb, self.matcher = (
            cv2.ORB_create(nfeatures=nfeatures),
    ...
    def update(self, next_3d, next_disp, next_img):
        ...
        if T is None and self.prev_img is not None:
    ...
    def point_clouds(self, kps1, kps2, desc1, desc2, im3d1, im3d2):
        ...
        for i in range(len(matches)):
            pts1[i] = bilinear_interpolate_pixels(im3d1, pts1[i][0], pts1[i][1])
            pts2[i] = bilinear_interpolate_pixels(im3d2, pts2[i][0], pts2[i][1])
        return np.array(pts1), np.array(pts2)

    def point_cloud_transform(self, current_pts, next_pts):
        if self.rigidity_threshold > 0:
            inlier_mask = rigid_body_filter(
                current_pts, next_pts, self.rigidity_threshold
            )
